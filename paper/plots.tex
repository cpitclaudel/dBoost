...
\begin{figure*}
  ...
  \caption{A comparison of various modeling approach on the Intel dataset. Figure~\ref{fig:gauss15} shows the results of simple uni-dimensional Gaussian modeling (Subsection~\ref{sec:gaus_model}). Figure~\ref{fig:gmm1t10} shows what happens when correlations are taken into account, using a multidimensional GMM with a single Gaussian (Subsection~\ref{sec:mixture_model}). Figure~\ref{fig:gmm2t05} shows the results of using two Gaussians instead of one to model the data. Note how the secondary data blobs, detected as outliers in the first two cases, are in the third case identified as valid data.}
\end{figure*}