...
In addition, when dealing with large amounts of data, it is possible -- and indeed, preferable -- to train the model on a random subset of the dataset before running the full analysis. This approach is particularly relevant when using the Mixture model, but the memory and time savings that it brings can of course be extended to all modeling methods.
