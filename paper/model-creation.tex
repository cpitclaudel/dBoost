...
In addition, when dealing with large amounts of data, it is possible -- and indeed, preferable -- to sample a subset of training data from the database to learn the model, and then analyze the entire database using this model. Although this approach is particularly relevant when using the Mixture model, it can of course be used for all modeling methods to save computation time.
