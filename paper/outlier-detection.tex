...
In order to take advantage of the correlations detected by the statistical analysis phase, dBoost adds to each expanded tuple an additional field for each set of correlated columns. This additional field contain the corresponding correlated values, and can be evaluated by all models capable of analyzing with multidimensional data.
