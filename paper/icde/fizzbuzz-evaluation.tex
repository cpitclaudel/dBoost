...
\srm{The example needs to be rewritten in the language of the introduction -- make it clear what the records in the database are, and what the expanded fields we add are.  The rule in below is misleading because it is not clear that it is actually expanding the fields.  We should instead say something like ``our library includes expansions that check for the divisibility of numbers by integers, and these rules make it easy to detect outliers in this example''.
}
